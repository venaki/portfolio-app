...
class TypeGroupCard extends StatelessWidget {
  ...
  const TypeGroupCard({super.key, required this.title, required this.items});
  ...
  double get totalProfitPct =>
      totalCost > 0 ? (totalProfit / totalCost) * 100 : 0.0;
  double get totalDailyChange =>
      items.fold(0.0, (s, e) => s + e.dailyChangeKRW);
  double get totalYestValue => items.fold(0.0, (s, e) => s + e.yestValueKRW);
  double get totalDailyPct =>
      totalYestValue > 0 ? (totalDailyChange / totalYestValue) * 100 : 0.0;
        ...
        children: [
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                Text(
                  item.hasPrice ? formatKRW(item.valueKRW) : '미평가',
                  style: const TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w600,
                    color: Color(0xFF1A1A1A),
                ...
                if (item.isComplete && item.costKRW > 0)
                  Text(
                ...
                if (item.isDailyComplete && item.dailyChangeKRW != 0)
                  Text(
