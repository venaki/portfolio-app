...
double calcCostKRW(Holding holding) {
  return holding.costKRW;
}

double calcProfitKRW(Holding holding, double currentPrice, double currentRate) {
  return calcTotalValueKRW(holding, currentPrice, currentRate) -
      calcCostKRW(holding);
}

double calcProfitPercentKRW(
  Holding holding,
  double currentPrice,
  double currentRate,
) {
  ...
  if (cost == 0) return 0;
  return ((calcTotalValueKRW(holding, currentPrice, currentRate) - cost) /
          cost) *
      100;
}
